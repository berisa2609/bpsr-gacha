      ...
      <div className="mx-auto max-w-5xl space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            「ジェム→非バインドルーノ購入」と「ガチャ期待値」を同じ前提で比較します。数字はすべて入力で変更できます。
          </p>
        </div>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="space-y-2">
                ...
                <div className="rounded-xl border p-3 text-sm">
                  <div>最安パック：<span className="font-medium">{best.pack?.name ?? "-"}</span></div>
                  <div>単価：<span className="font-medium">{fmt(best.unit, 4)}</span> 円/ジェム</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Card className="rounded-2xl">
                ...
                <CardContent className="space-y-2 text-sm">
                  <div className="flex items-center justify-between"><span>80連 必要ジェム</span><span className="font-medium">{fmt(quick.need80, 0)}</span></div>
                  <div className="flex items-center justify-between"><span>最小課金</span><span className="font-medium">{yen(quick.min80.costYen)}</span></div>
                  <div className="text-xs text-muted-foreground">内訳：{packs.map((p, i) => `${p.name}×${quick.min80.counts[i]}`).join(" / ")}</div>
                  <Separator className="my-2" />
                  <div className="flex items-center justify-between"><span>100連 必要ジェム</span><span className="font-medium">{fmt(quick.need100, 0)}</span></div>
                  <div className="flex items-center justify-between"><span>最小課金</span><span className="font-medium">{yen(quick.min100.costYen)}</span></div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Card className="rounded-2xl">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="rounded-xl border p-3 text-sm space-y-1">
                    <div className="flex items-center justify-between"><span>必要ルーノ合計</span><span className="font-medium">{fmt(willBuy.totalRuno, 0)}</span></div>
                    <div className="flex items-center justify-between"><span>必要ジェム</span><span className="font-medium">{fmt(willBuy.gemsNeeded, 0)}</span></div>
                    <div className="flex items-center justify-between"><span>理論額（最安単価換算）</span><span className="font-medium">{yen(willBuy.theo)}</span></div>
                    <div className="flex items-center justify-between"><span>最小課金（パック）</span><span className="font-medium">{yen(willBuy.minPay.costYen)}</span></div>
                    <div className="text-xs text-muted-foreground">内訳：{packs.map((p, i) => `${p.name}×${willBuy.minPay.counts[i]}`).join(" / ")}（受取 {fmt(willBuy.minPay.gems, 0)} ジェム）</div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="rounded-2xl">
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="rounded-xl border p-3 text-sm space-y-1">
                    <div className="flex items-center justify-between"><span>A以上</span><span className="font-medium">{fmt(willGacha.pAplus * 100, 4)}%</span></div>
                    <div className="flex items-center justify-between"><span>A（Sを除く）</span><span className="font-medium">{fmt(willGacha.pA * 100, 4)}%</span></div>
                    <div className="flex items-center justify-between"><span>欲しいAが出る確率</span><span className="font-medium">{fmt(willGacha.pDesiredA * 100, 4)}%</span></div>
                    <Separator className="my-2" />
                    <div className="flex items-center justify-between"><span>期待ガチャ回数</span><span className="font-medium">{fmt(willGacha.expectedPulls, 0)}回</span></div>
                    <div className="flex items-center justify-between"><span>期待ジェム</span><span className="font-medium">{fmt(willGacha.expectedGems, 0)}</span></div>
                    <div className="flex items-center justify-between"><span>理論額（最安単価換算）</span><span className="font-medium">{yen(willGacha.theo)}</span></div>
                    <div className="flex items-center justify-between"><span>最小課金（パック）</span><span className="font-medium">{yen(willGacha.minPay.costYen)}</span></div>
                    <div className="text-xs text-muted-foreground">内訳：{packs.map((p, i) => `${p.name}×${willGacha.minPay.counts[i]}`).join(" / ")}（受取 {fmt(willGacha.minPay.gems, 0)} ジェム）</div>
                  </div>

                  <div className="rounded-xl border p-3 text-sm">
                    <div className="font-medium mb-1">比較（最小課金ベース）</div>
                    <div className="flex items-center justify-between"><span>購入（ルーノ）</span><span className="font-medium">{yen(compare.buyYen)}</span></div>
                    <div className="flex items-center justify-between"><span>ガチャ（期待値）</span><span className="font-medium">{yen(compare.gachaYen)}</span></div>
                    <div className="flex items-center justify-between"><span>差（ガチャ−購入）</span><span className={"font-medium " + (compare.diff >= 0 ? "text-red-600" : "text-emerald-600")}>
                      {compare.diff >= 0 ? "+" : ""}{yen(compare.diff)}
                    </span></div>
            ...
            <Card className="rounded-2xl">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="rounded-2xl border p-4 text-sm space-y-2">
                  <div className="flex items-center justify-between"><span>A確率</span><span className="font-medium">{fmt(costume.pA * 100, 4)}%</span></div>
                  <div className="flex items-center justify-between"><span>期待ガチャ回数</span><span className="font-medium">{fmt(costume.expectedPulls, 0)}回</span></div>
                  <div className="flex items-center justify-between"><span>期待ジェム</span><span className="font-medium">{fmt(costume.expectedGems, 0)}</span></div>
                  <div className="flex items-center justify-between"><span>理論額（最安単価換算）</span><span className="font-medium">{yen(costume.theo)}</span></div>
                  <div className="flex items-center justify-between"><span>最小課金（パック）</span><span className="font-medium">{yen(costume.minPay.costYen)}</span></div>
                  <div className="text-xs text-muted-foreground">内訳：{packs.map((p, i) => `${p.name}×${costume.minPay.counts[i]}`).join(" / ")}（受取 {fmt(costume.minPay.gems, 0)} ジェム）</div>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
        ...
        <div className="pb-8 text-xs text-muted-foreground">
          注：このアプリは「期待値（平均）」中心です。確率のばらつき（例えば90%で到達など）も必要なら追加できます。
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border p-3 text-sm">
        ...
        <div className="text-xs text-muted-foreground mt-1">受取 {fmt(res.minPay.gems, 0)} ジェム（余り {fmt(res.minPay.gems - ceilInt(res.gems), 0)} ジェム）</div>
      </div>
